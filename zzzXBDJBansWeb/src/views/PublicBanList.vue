<template>
    <div class="min-h-screen bg-gradient-to-br from-gray-100 via-white to-gray-100 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 text-slate-900 dark:text-slate-200 font-sans selection:bg-rose-500/30 transition-colors duration-300">
        <div class="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8 space-y-8">
            <!-- Header -->
            <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-gray-200 dark:border-slate-800/60 pb-8">
                <div>
                ...
                <div>
                     <router-link to="/" class="group inline-flex items-center px-5 py-2.5 border border-gray-200 dark:border-slate-700 rounded-xl shadow-sm text-sm font-semibold text-slate-600 dark:text-slate-300 bg-white dark:bg-slate-800/50 hover:bg-gray-50 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-white hover:border-gray-300 dark:hover:border-slate-600 transition-all duration-300">
            ...
            <div class="sticky top-4 z-20">
                <div class="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-gray-200 dark:border-slate-800 rounded-2xl shadow-xl dark:shadow-2xl p-4 flex flex-col md:flex-row gap-4 justify-between items-center transition-colors">
                    <!-- Status Filter (Active/Expired) -->
                    <div class="flex p-1 space-x-1 bg-gray-100 dark:bg-slate-950/50 rounded-xl border border-gray-200 dark:border-slate-800/50">
                        <button 
                            @click="currentTab = 'all'"
                            :class="[
                                'px-4 py-2 text-sm font-medium rounded-lg transition-all',
                                currentTab === 'all' ? 'bg-white dark:bg-slate-800 text-slate-900 dark:text-white shadow-sm ring-1 ring-gray-200 dark:ring-white/10' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'
                            ]"
                        >
                            全部
                        </button>
                        <button 
                            ...
                            :class="[
                                ...
                                currentTab === 'active' ? 'bg-rose-50 dark:bg-rose-900/30 text-rose-600 dark:text-rose-400 shadow-sm ring-1 ring-rose-500/20' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'
                            ]"
                        >
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-800/50">
                            <thead class="bg-gray-50 dark:bg-slate-950/50">
                                <tr>
                                    <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">状态</th>
                                    <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">玩家信息</th>
                                    <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">封禁原因</th>
                                    <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">时长 / 过期时间</th>
                                    <th scope="col" class="px-6 py-4 text-right text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">执行时间</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-200 dark:divide-slate-800/50 bg-white/50 dark:bg-slate-900/20">
                                <tr v-for="item in publicBans" :key="item.id" class="hover:bg-rose-50 dark:hover:bg-rose-500/5 transition-colors group">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex flex-col">
                                            <div class="flex items-center gap-2">
                                                <div class="text-sm font-bold text-slate-900 dark:text-white group-hover:text-rose-600 dark:group-hover:text-rose-300 transition-colors">{{ item.name }}</div>
                                                <a 
                                            ...
                                            <div class="flex flex-col gap-1 mt-1.5">
                                                ...
                                                <div v-if="item.steam_id_64" class="flex items-center gap-2 group/id">
                                                    ...
                                                    <span class="font-mono text-xs text-slate-500 dark:text-slate-400 select-all group-hover/id:text-slate-700 dark:group-hover/id:text-slate-200 transition-colors">{{ item.steam_id_64 }}</span>
                                                    <button @click="copyToClipboard(item.steam_id_64)" class="opacity-0 group-hover/id:opacity-100 text-slate-400 hover:text-indigo-500 transition-all" title="复制">
                                                        <svg class="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                ...
                                                <div v-if="item.steamId" class="flex items-center gap-2 group/id">
                                                    ...
                                                    <span class="font-mono text-xs text-slate-500 select-all group-hover/id:text-slate-700 dark:group-hover/id:text-slate-400 transition-colors">{{ item.steamId }}</span>
                                                </div>
                                                <!-- ID3 -->
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="text-sm text-slate-700 dark:text-slate-300 max-w-xs truncate" :title="item.reason">{{ item.reason || '无原因' }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex flex-col">
                ...
                <div v-if="hasResults" class="md:hidden grid grid-cols-1 gap-4">
                    <div v-for="item in publicBans" :key="item.id" class="bg-white/60 dark:bg-slate-900/60 backdrop-blur-md rounded-xl border border-gray-200 dark:border-slate-800 p-5 shadow-lg relative overflow-hidden group">
                        ...
                        <div class="flex justify-between items-start mb-3 pl-3">
                             <div>
                                <div class="flex items-center gap-2">
                                    <h3 class="text-lg font-bold text-slate-900 dark:text-slate-100 group-hover:text-rose-600 dark:group-hover:text-rose-400 transition-colors">{{ item.name }}</h3>
                                    <a 
                                        v-if="item.steam_id_64"
                                        :href="`https://steamcommunity.com/profiles/${item.steam_id_64}`" 
                                        target="_blank"
                        ...
                        <div class="pl-3 space-y-3 mt-4 border-t border-gray-100 dark:border-slate-800/50 pt-3">
                            <div>
                                <span class="text-[10px] text-slate-500 uppercase tracking-wider block mb-1">封禁原因</span>
                                <p class="text-sm text-slate-700 dark:text-slate-300">{{ item.reason || '无原因' }}</p>
                            </div>
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <span class="text-[10px] text-slate-500 uppercase tracking-wider block mb-1">时长</span>
                                    <span class="text-sm text-slate-900 dark:text-white">{{ formatDuration(item.duration) }}</span>
                                </div>
                                <div>
                        ...
                        <button
                            ...
                            :class="[
                                currentPage === p 
                                    ...
                                    : 'bg-white dark:bg-slate-900 text-slate-500 dark:text-slate-400 border-gray-200 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-white',
...
const getStatusBadgeClass = (status) => {
    if (status === 'active') {
        return 'bg-rose-50 dark:bg-rose-500/10 text-rose-600 dark:text-rose-400 border-rose-200 dark:border-rose-500/20 shadow-rose-500/10';
    }
    return 'bg-gray-100 dark:bg-slate-700/30 text-slate-500 dark:text-slate-400 border-gray-200 dark:border-slate-600/30';
};
